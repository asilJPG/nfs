...
Рассылка не ждёт планировщик: очередь стартует сразу после создания и сама тянет
следующий батч, пока есть адресаты (~25 сообщений/с, обработка 429 и блокировок).
`vercel.json` держит два суточных задания — страховку для очереди и уборку
(сгорание наград, чистка использованных токенов). Суточные, потому что Hobby
отклоняет деплой с более частым расписанием.
