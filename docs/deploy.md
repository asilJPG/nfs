...
Hobby разрешает cron не чаще раза в сутки, причём деплой с минутным расписанием
отклоняется целиком — с ошибкой «Hobby accounts are limited to daily cron jobs».
Поэтому в vercel.json оба задания суточные, а рассылка на планировщик не
опирается:

- очередь стартует сразу после создания рассылки;
- отправив батч, эндпоинт вызывает сам себя, пока остаются адресаты;
- суточный запуск остаётся страховкой, если цепочка оборвалась.

Vercel сам подставляет `Authorization: Bearer $CRON_SECRET` в свои cron-запросы;
тем же заголовком защищён и внутренний вызов очереди.

На Pro можно вернуть `* * * * *` — поведение не изменится, просто страховка
станет срабатывать чаще.
