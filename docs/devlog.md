...
Компактный справочник по состоянию проекта. Дата актуализации — 11 сентября 2026.
...
| `/staff` | бариста | касса: скан QR + дневные KPI + недельный график + «на 1 до награды» + список последних касаний с именем гостя и прогрессом |
...
- `NEXT_PUBLIC_SUPPORT_TELEGRAM`, `NEXT_PUBLIC_SUPPORT_EMAIL` — контакты
  поддержки; без них ссылка ведёт на сам бот, e-mail скрывается (`lib/contact.ts`)
- `NEXT_PUBLIC_TENANT_TZ` / `NEXT_PUBLIC_TENANT_TZ_OFFSET` — таймзона арендатора
  (по умолчанию `Asia/Tashkent` / `+05:00`; используется в `lib/time.ts`)
...
Вход: `/login`, логин `donikboychik`, пароль `uhahahablin` → редирект на
`/admin`. Смена одноразовым скриптом (`scripts/admin-password.ts <login>
<pass>` — только пароль; смена логина через `updateUserById(email)`).
...
## Пре-лонч аудит (11 сентября 2026)

Прогнан второй аудит из 30 находок перед стартом. **CRITICAL + HIGH
закрыты полностью**, MEDIUM закрыт частично, LOW отложен.

**CRITICAL (3/3):**
- C1: реальные бренды кофеен в лендинге/live-preview/apply-плейсхолдерах
  → нейтральные (Aroma/Corner/Rise/Loft/Terrace/Pekarnya/Panorama)
- C2: `startapp=tap_<slug>` в проде больше не штампует (кто угодно
  штамповал бы себе бесплатно). Работает как `t_` — просто открывает
  карту. Штампует только под `STAMPY_DEV_MODE=1`
- C3: заменён дефолтный `admin/admin12` на `donikboychik/uhahahablin`

**HIGH (5/5):**
- H1: `removeStaff` теперь проверяет роль цели — управляющий не отключит
  владельца или другого управляющего
- H2: iOS Safari — jsQR fallback для сканера (BarcodeDetector API там нет)
- H3: `/login` — 8 попыток/мин на IP+логин (in-memory sliding window
  в `lib/rateLimit.ts`)
- H4: `/api/miniapp/state` — 60/мин, `/api/miniapp/redeem` — 20/мин по
  telegram_id
- H5: open-redirect в `stopImpersonatingAction` (`//evil` больше не проходит)

**MEDIUM (6/11):**
- M1: MRR-плитке свой спарклайн (кумулятивная серия по платящим), не
  общая seriesTenants
- M2: MRR исключает тенантов с истёкшим `subscription_until`
- M3: «Активность % базы за сегодня» → «Возвраты · гостей базы» (было
  непонятно что за метрика)
- M5/M6: контакты через env (`NEXT_PUBLIC_SUPPORT_TELEGRAM`/`_EMAIL`),
  fallback на самого бота
- M11: плитка «Все системы работают» реально смотрит на результат
  селектов страницы — при ошибке краснеет
- M13: `/auth/signout` чистит `__Host-stampy_impersonate`, иначе
  следующий логин подхватывал подмену
- M14: `/apply` дополнительно лимитируется по IP (10/час), не только
  по телефону

**Остались** (не блокеры): M4/M8 (уже помечены как «пример»/«демо»),
M9 (Notifications таб синтезированный), M12 (Profile-заглушки в
мини-аппе — тумблеры без обработчиков), 8× LOW.
...
Отдельный светлый экран под планшет на прилавке; исторически был обёрнут
в тёмный фрейм «под iPad» с четырьмя нерабочими табами
(Штампы/Награды/Аналитика/История подсвечивались, контент не переключался).
Перекроен 11 сентября:

- Полноэкранная раскладка (240px сайдбар + main), без мокапного фрейма
- Табы удалены — один рабочий экран смены. Аналитика и история живут
  в `/dashboard` у владельца
- Владельцу/менеджеру видна пилюля «Вернуться в кабинет»; бариста нет
  (это его единственный экран)
- Кнопка «Сменить смену» (POST `/auth/signout`) — иначе на общем
  планшете все касания висят на утреннем логине
- Иконка в `DashboardNav` для «Касса бариста» — `IconScan` (была молния)

**Данные и таймзона.** Vercel-функции живут в `hnd1` (UTC+9), Ташкент
UTC+5 — раньше `new Date(y, m, d)` считало «сегодня» токийским часом,
вечерние касания уезжали в «завтра», время событий печаталось в
токийском. Ввёл `lib/time.ts` с явной таймзоной арендатора:
`tenantDayStart`/`tenantDayEnd` строятся через ISO с фиксированным
offset, `formatTenantTime`/`formatTenantDate` через `Intl.DateTimeFormat`.

**Навигация по датам.** `?date=YYYY-MM-DD` — валидируется формат и не
даёт заглядывать в будущее. В шапке кнопки «Предыдущий день / Сегодня /
Следующий день»; спарклайн и события пересчитываются относительно
выбранного дня.

**KPI.** Плитки катят цифры от прошлого значения к новому через
`components/ui/CountUp.tsx` (RAF, ease-out 700мс). У плитки штампов
дельта к предыдущему дню (`+3 vs вчера` / `−2 vs вчера` / `как вчера`).

**График.** Под линией подписи дней (пн/вт/…) с последним жирным;
в заголовке сумма недели.

**События.** Присоединяют гостя (`stampy_customers.first_name` →
`@username` → «Гость») и его прогресс `stamps_count/stamps_required` из
активной программы. «Штамп добавлен · NFC-метка · TEST1» превратилось в
«Азиз · 5/6 · NFC · shemelis». Не-сегодняшние события помечаются датой.

**«На один штамп до награды».** Топ-10 memberships по накопленным
штампам, фильтруются клиентски по `stamps_required - 1`, показываются
до 5. Полезная подсказка бариста: «Азиз пришёл, у него 5/6».
...
- `loading.tsx` во всех разделах — вместо белого экрана моментально скелетон.
  `/staff/loading.tsx` собран под светлую полноэкранную кассу (общие
  `SkeletonBlock` тёмные, на белом фоне не видны); `/admin/loading.tsx`
  переписан под таблицу кофеен вместо старых 4+4 плиток
...
- `/admin/*` (кроме overview) — не под общими токенами, много хардкода
  `#14161D`/`#5B8DEF`
- Churn в `/admin` — пока захардкожено (MRR уже честный, см. пре-лонч
  M1/M2)
...
- Kаccа: «готовы забрать награду» (гости с накопленной, но не съеденной),
  поиск гостя, ручной +штамп с причиной, звук/toast на успешный скан —
  предлагал, ждём отмашку
