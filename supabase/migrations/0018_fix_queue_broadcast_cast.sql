-- 0016 забыл каст к enum broadcast_status в финальном update. Перевыпуск функции.
...
begin
  ...
  update stampy_broadcasts
     set status = (case when scheduled_at is null or scheduled_at <= now() then 'sending' else 'scheduled' end)::broadcast_status,
